...
export const GroupedFrequenciesModule: React.FC<GroupedFrequenciesModuleProps> = ({
  ...
  return (
    <div ref={containerRef} className="space-y-6">
      ...
      {/* Ventana Flotante / Modal a Pantalla Completa para Frecuencias Agrupadas */}
      <FloatingTableModal
        isOpen={isFloatingTableOpen}
        onClose={() => setIsFloatingTableOpen(false)}
        title={`Frecuencias Agrupadas: ${data.variableName}`}
        subtitle={`Muestra n = ${data.sampleSize} • k = ${data.parameters.k} intervalos • Amplitud A = ${data.parameters.amplitud}`}
        badge={`k = ${data.parameters.k}`}
        onExportExcel={() => exportGroupedTableToExcel(data)}
      >
        {renderTableContent()}
      </FloatingTableModal>

      {/* Ficha Flotante / Bottom Sheet por Fila para Móviles */}
